        ...
        <section id='writingsWrapper'>
            ...
            <section>
                <article>
                    ...
                    <p>
                        The feedback required manifests itself in two ways. The first is that an error message should be presented informing the user what went wrong. These messages are declared within the model's class and should be placed into the <code>flash</code> hash-map whenever a violation occurs. Which key each message is associated with is up to the developer and defined within the relevant controller action. In the example of the <code>create</code> controller action, <code>flash</code> is occupied dependent on whether or not <code>@user.valid?</code> returns true. In the case in which it returns false, <code>@user.errors</code> is placed into <code>flash[:login]</code>. The schema of <code>@user.errors</code> is <code>{:&lt;model attribute&gt; =&gt; &lt;error message&gt;, ... }</code>.
                    </p>
                    <p>
